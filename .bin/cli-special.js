...
const toCamelCased = s =>
  s.replace (/_([a-z])/g, function (g) {
...
const createModel = async (name, props) => {
  ...
  const params = props
    ...
    .map (item => ({
      ...
      name: toCamelCased (item.split (':')[0]),
      type: item.split (':')[1],
    }));
  const fileContent = fs.readFileSync (
    path.join (__dirname, 'tmp/model.ejs'),
    'utf8'
  );
  ...
  fs.writeFileSync (
    path.join (__dirname, `../src/models/${toFileName (name)}.model.ts`),
    content
  );
};
const createService = async (name, props) => {
  ...
  const fileContent = fs.readFileSync (
    path.join (__dirname, 'tmp/service.ejs'),
    'utf8'
  );
  const content = ejs.render (fileContent, {
    name,
  ...
  fs.writeFileSync (
    path.join (__dirname, `../src/services/${toFileName (name)}.service.ts`),
    content
  );
};
const createController = async (name, props) => {
  ...
  const fileContent = fs.readFileSync (
    path.join (__dirname, 'tmp/controller.ejs'),
    'utf8'
  );
  const content = ejs.render (fileContent, {
    name,
  ...
  fs.writeFileSync (
    path.join (
      __dirname,
      `../src/controllers/${toFileName (name)}.controller.ts`
    ),
    content
  );
};
const createFactory = async (name, props) => {
  ...
  const fileContent = fs.readFileSync (
    path.join (__dirname, 'tmp/factory.ejs'),
    'utf8'
  );
  const content = ejs.render (fileContent, {
    name,
  ...
  fs.writeFileSync (
    path.join (__dirname, `../src/database/factories/${name}.factory.ts`),
    content
  );
};
const createSeed = async (name, props) => {
  ...
  const fileContent = fs.readFileSync (
    path.join (__dirname, 'tmp/seed.ejs'),
    'utf8'
  );
  const content = ejs.render (fileContent, {
    name,
  ...
  fs.writeFileSync (
    path.join (__dirname, `../src/database/seeds/create-${name}.seed.ts`),
    content
  );
};
...
(async () => {
  ...
  if (args[0] === 'g' || args[0] === 'generate') {
    if (args[1] === 'm' || args[1] === 'model') {
      if (!args[2]) {
        console.log ('name model require!');
      }
