  ...
  static findByCond(query: any) {
    ...
    if (query.search && query.search.length > 0) {
      for (let i = 0; i < query.search.length; i++) {
        ...
        if (i === 0) {
          queryBuilder.where(`events.${element.key} ${element.opt} :${i}`).setParameter(i.toString(), element.value);
        } else {
          queryBuilder.andWhere(`events.${element.key} ${element.opt} :${i}`).setParameter(i.toString(), element.value);
        }
      }
    }
